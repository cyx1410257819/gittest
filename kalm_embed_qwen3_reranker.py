...
app = FastAPI(
    ...
    description="基于 KaLM-Embedding-2.5 与 Qwen3-Reranker 的两阶段文档排序接口",
)
...
class EmbeddingReranker:
    ...
    def initialize(self):
        try:
            ...
            self.model = LLM(
                model="/root/cyx/model_weights/KaLM-Embedding-2.5",  # 更换为KaLM模型路径
                task="embed",
                tensor_parallel_size=num_gpus if num_gpus > 0 else 1,
                gpu_memory_utilization=0.15,
                trust_remote_code=True,  # KaLM模型可能需要此参数
                dtype="float16",  # 根据KaLM模型要求设置数据类型
            )
...
async def health_check():
    return {"status": "healthy", "models": ["KaLM-Embedding-2.5", "Qwen3-Reranker-4B"]}
...
if __name__ == "__main__":
    import uvicorn
    uvicorn.run(app, host="0.0.0.0", port=8073, workers=1)
